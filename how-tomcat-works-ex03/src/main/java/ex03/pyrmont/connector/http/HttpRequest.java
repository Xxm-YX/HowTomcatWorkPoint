...
public class HttpRequest implements HttpServletRequest {
    ...
    /**
     * The authorization credentials send with this Request
     */
     ...
     */
    protected void parseParameters() throws UnsupportedEncodingException {
        if (parsed){
            return;
        }
        ...
        // Parse any parmeters specified in the query string
        //查询的字符串
        String queryString = getQueryString();
        //先转码
        RequestUtil.parseParameters(results, queryString, encoding);
//        Parameters parameters1 = new Parameters();
//        parameters1.setEncoding(encoding);
//        parameters1.getParameter()
        ...
        // 这里判断是不是POST请求，并且判断请求的数据类型
        if("POST".equals(getMethod()) && (getContentLength() > 0)
        && "application/x-www-form-urlencoded".equals(contentType)) {
            try{
                //contentLength() 不包含请求头的长度，只包含实体主体的长度
                int max = getContentLength();
                int len = 0;
                byte buf[] = new byte[getContentLength()];
                RequestStream is = (RequestStream) createInputStream();
                while( len < max) {
                    // 返回读取了多少个字符
     ...
     * instance of RequestStream associated with request, but this can
    ...
    public long getDateHeader(String name) {
        ...
        // Attempt to convert the date header in a veriety of formats
